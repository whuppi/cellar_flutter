...
| Publish to pub.dev | PLANNED | Maintainer-gated. The swap tooling is shipped (`tool/ci/release_hooks.sh`): the dep becomes `^<pin's release tag>` and the release aborts on an untagged pin — cellar releases first |
